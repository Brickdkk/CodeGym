...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Stop-OnError "Fallo la instalacion de dependencias"
    }
    Write-Success "Dependencias instaladas correctamente"
} catch {
    Stop-OnError "Error ejecutando npm install: $($_.Exception.Message)"
}
...
try {
    ...
    try {
        ...
        if ($healthResponse.StatusCode -eq 200) {
            Write-Success "Health check endpoint respondio correctamente (200 OK)"
...
🚀 EL PROYECTO ESTA LISTO PARA DEPLOY EN RENDER 🚀

📊 Resumen de Validacion:
   • Pruebas Pasadas: $Global:TestsPassed
   • Pruebas Fallidas: $Global:TestsFailed
   • Duracion Total: $($duration.TotalSeconds.ToString("F1")) segundos
...
⚠️  IMPORTANTE: Asegurese de que las variables de entorno esten 
